...
void Manager::RefreshLists(std::string_view a_signatures) {
    ...
    if (includes("NPC_")) PopulateList<RE::TESNPC>("NPC", [](RE::TESNPC* npc) { return npc != nullptr; });
}
...
extern json GetEffectiveSettings(int targetLevel, RE::Actor* actor);
...
static RE::ActorValue ResolveVanillaSkillActorValue(const std::string& skillId) {
    static const std::map<std::string, RE::ActorValue> values{
    ...
    const auto found = values.find(skillId);
    return found == values.end() ? RE::ActorValue::kNone : found->second;
}

static float GetActorLevelXPThreshold(int level) {
    float base = 75.0f;
    float multiplier = 25.0f;
    if (const auto settings = RE::GameSettingCollection::GetSingleton()) {
        if (const auto value = settings->GetSetting("fXPLevelUpBase")) base = value->GetFloat();
        if (const auto value = settings->GetSetting("fXPLevelUpMult")) multiplier = value->GetFloat();
    }
    return std::max(1.0f, base + multiplier * static_cast<float>(std::max(1, level)));
}
...
void Manager::SetActorXP(RE::Actor* actor, const std::string& skillId, float xp) {
    ...
    const auto dataIt = customSkillsData.find(skillId);
    if (actor->IsPlayerRef() && (dataIt == customSkillsData.end() || !dataIt->second.isVanilla)) {
...
void Manager::SyncLegacyPlayerStateToActorValues() {
    ...
    for (const auto& [skillId, state] : playerCustomSkills) {
        if (!customSkillsData.contains(skillId) || customSkillsData.at(skillId).isVanilla) continue;
...
void Manager::LoadCustomSkills() {
    ...
    customSkillsData.clear();

    for (const auto& j : configs) {
        if (j.contains("isVanilla")) {
            CustomSkill skill;
            ...
            skill.isVanilla = j.value("isVanilla", false);
            if (skill.isVanilla && ResolveVanillaSkillActorValue(skill.id) == RE::ActorValue::kNone) continue;
            skill.advancesPlayerLevel = j.value("advancesPlayerLevel", skill.isVanilla);
            ...
            if (!skill.isVanilla && playerCustomSkills.find(skill.id) == playerCustomSkills.end()) {
                playerCustomSkills[skill.id] = { skill.initialLevel, GetActorXP(RE::PlayerCharacter::GetSingleton(), skill.id), 0 };
            }

            logger::debug("Skill carregada: {} (vanilla={} display={})", skill.id, skill.isVanilla, skill.displayName);
        }
    }
...
int Manager::GetCustomSkillLevel(RE::Actor* actor, const std::string& skillId) {
    ...
    const bool vanilla = dataIt != customSkillsData.end() && dataIt->second.isVanilla;
    RE::ActorValue av = vanilla ? ResolveVanillaSkillActorValue(skillId) : ResolveCustomSkillActorValue(skillId);
...
void Manager::SetCustomSkillLevel(RE::Actor* actor, const std::string& skillId, int level) {
    ...
    RE::ActorValue av = vanilla ? ResolveVanillaSkillActorValue(skillId) : ResolveCustomSkillActorValue(skillId);
    auto avOwner = actor->AsActorValueOwner();
    if (vanilla && !actor->IsPlayerRef() && av != RE::ActorValue::kNone && avOwner) {
        auto values = FollowerDistribution::Contributions(actor);
        const auto name = std::string(RE::ActorValueList::GetActorValueName(av));
        const float currentTotal = avOwner->GetPermanentActorValue(av);
        values[name] += static_cast<float>(level) - currentTotal;
        if (!FollowerDistribution::Apply(actor, FollowerDistribution::Purchased(actor), std::move(values), {})) {
            logger::warn("[SkillLevel] Distribution unavailable actor={:08X} skill={}", actor->GetFormID(), skillId);
        }
        return;
    }
    if (av != RE::ActorValue::kNone && avOwner) {
...
float Manager::GetCustomSkillXP(RE::Actor* actor, const std::string& skillId) {
    const auto dataIt = customSkillsData.find(skillId);
    if (actor && actor->IsPlayerRef() && dataIt != customSkillsData.end() && dataIt->second.isVanilla) {
        const auto av = ResolveVanillaSkillActorValue(skillId);
        auto player = RE::PlayerCharacter::GetSingleton();
        auto skills = player ? player->GetPlayerRuntimeData().skills : nullptr;
        const auto index = static_cast<std::uint32_t>(av);
        if (skills && skills->data && index >= 6 && index <= 23) return skills->data->skills[index - 6].xp;
    }
...
int Manager::GetCustomSkillBonus(RE::Actor* actor, const std::string& skillId) {
    ...
    auto avOwner = actor->AsActorValueOwner();
    if (av != RE::ActorValue::kNone && avOwner) {
        if (vanilla && !actor->IsPlayerRef()) {
            const auto name = std::string(RE::ActorValueList::GetActorValueName(av));
            const auto values = FollowerDistribution::Contributions(actor);
            const auto found = values.find(name);
            return found == values.end() ? 0 : static_cast<int>(std::round(found->second));
        }
...
void Manager::SetCustomSkillBonus(RE::Actor* actor, const std::string& skillId, int amount) {
    ...
    RE::ActorValue av = vanilla ? ResolveVanillaSkillActorValue(skillId) : ResolveCustomSkillActorValue(skillId);
    auto avOwner = actor->AsActorValueOwner();
    if (vanilla && !actor->IsPlayerRef() && av != RE::ActorValue::kNone) {
        auto values = FollowerDistribution::Contributions(actor);
        const auto name = std::string(RE::ActorValueList::GetActorValueName(av));
        if (amount == 0) values.erase(name);
        else values[name] = static_cast<float>(amount);
        if (!FollowerDistribution::Apply(actor, FollowerDistribution::Purchased(actor), std::move(values), {})) {
            logger::warn("[SkillBonus] Distribution unavailable actor={:08X} skill={}", actor->GetFormID(), skillId);
        }
        return;
    }
    if (av != RE::ActorValue::kNone && avOwner) {
...
int Manager::GetActorProgressionLevel(RE::Actor* actor) {
    if (!actor) return 1;
    if (actor->IsPlayerRef()) return std::max(1, static_cast<int>(actor->GetLevel()));
    auto& state = EnsureActorProgress(actor);
    return std::max({
        1,
        static_cast<int>(actor->GetLevel()),
        state.highestRewardedLevel + state.pendingLevelUps
    });
}

float Manager::GetActorLevelProgress(RE::Actor* actor) {
    if (!actor || actor->IsPlayerRef()) return 0.0f;
    const auto& state = EnsureActorProgress(actor);
    const float threshold = GetActorLevelXPThreshold(GetActorProgressionLevel(actor));
    return threshold > 0.0f ? std::clamp(state.skillLevelXP / threshold * 100.0f, 0.0f, 100.0f) : 0.0f;
}

void Manager::AddFollowerSkillLevelXP(
    RE::Actor* actor,
    const CustomSkill& skill,
    const std::vector<int>& gainedLevels)
{
    if (!actor || actor->IsPlayerRef() || gainedLevels.empty() || !skill.advancesPlayerLevel) return;
    const auto settings = GetSettings();
    if (!settings.value("followerDetection", json::object()).value("skillXPAdvancesLevel", false)) return;

    auto& state = EnsureActorProgress(actor);
    for (const int level : gainedLevels) state.skillLevelXP += static_cast<float>(std::max(0, level));

    int progressionLevel = GetActorProgressionLevel(actor);
    while (state.pendingLevelUps < 10000) {
        const float threshold = GetActorLevelXPThreshold(progressionLevel);
        if (state.skillLevelXP + 0.001f < threshold) break;
        state.skillLevelXP -= threshold;
        state.pendingLevelUps++;
        progressionLevel++;
    }
    state.lastObservedLevel = std::max(state.lastObservedLevel, progressionLevel);
}
    ...
    std::vector<PaidResource> resources)
{
    ...
    record.actorLevelAtPurchase = GetActorProgressionLevel(actor);
...
void Manager::ModCustomSkillLevelForActorID(RE::FormID actorFormID, const std::string& skillId, int amount) {
    ...
    const auto dataIt = customSkillsData.find(skillId);
    const int currentLevel = dataIt != customSkillsData.end() && dataIt->second.isVanilla ?
        GetCustomSkillTotalLevel(actor, skillId) : GetCustomSkillLevel(actor, skillId);
    const auto nextLevel = std::clamp<std::int64_t>(
        static_cast<std::int64_t>(currentLevel) + amount,
        0,
...
void Manager::AddCustomSkillXPForActor(RE::Actor* actor, const std::string& skillId, float xpAmount) {
    ...
    const auto skillData = skillDataIt->second;

    if (skillData.isVanilla && !actor->IsPlayerRef() && FollowerDistribution::Busy(actor)) {
        if (FollowerDistribution::Loading()) {
            logger::warn(
                "[SkillXP] Ignored during save load actor={:08X} skill={}",
                actor->GetFormID(),
                skillId);
            return;
        }
        const auto actorID = actor->GetFormID();
        const auto generation = FollowerDistribution::Epoch();
        SKSE::GetTaskInterface()->AddTask([actorID, skillId, xpAmount, generation]() {
            if (generation != FollowerDistribution::Epoch()) return;
            Manager::GetSingleton()->AddCustomSkillXPForActorID(actorID, skillId, xpAmount);
        });
        return;
    }

    if (skillData.isVanilla && actor->IsPlayerRef()) {
        const auto av = ResolveVanillaSkillActorValue(skillId);
        if (av != RE::ActorValue::kNone) {
            const float finalXP = xpAmount * skillData.expFormula.useMult + skillData.expFormula.useOffset;
            if (std::isfinite(finalXP) && finalXP > 0.0f) {
                RE::PlayerCharacter::GetSingleton()->AddSkillExperience(av, finalXP);
            }
            Prisma::SendUpdateToUI();
        }
        return;
    ...
    std::vector<int> gainedLevels;
    float committedXP = 0.0f;

    {
        std::lock_guard<std::mutex> updateLock(_notificationMutex);

        auto& data = customSkillsData.at(skillId);
        dispName = data.displayName;

        int currentLevel = data.isVanilla ?
            GetCustomSkillTotalLevel(actor, skillId) : GetCustomSkillLevel(actor, skillId);
        ...
        const auto settings = GetEffectiveSettings(GetActorProgressionLevel(actor), actor);
        int maxCap = settings.value("skillCap", 100);
        ...
        while (currentXP >= (reqXp - 0.001f) && currentLevel < maxCap) {
            ...
            currentLevel++;
            gainedLevels.push_back(currentLevel);
        ...
        committedXP = currentXP;

        if (data.isVanilla && !actor->IsPlayerRef()) {
            if (gainedLevels.empty()) {
            ...
            auto values = FollowerDistribution::Contributions(actor);
            const auto av = ResolveVanillaSkillActorValue(skillId);
            if (av == RE::ActorValue::kNone) return;
            const auto name = std::string(RE::ActorValueList::GetActorValueName(av));
            values[name] += static_cast<float>(gainedLevels.size());
            const auto actorID = actor->GetFormID();
            if (!FollowerDistribution::Apply(
                    actor,
                    FollowerDistribution::Purchased(actor),
                    std::move(values),
                    [actorID, skillId, committedXP, gainedLevels, skillData](bool ok) {
                        if (!ok) return;
                        SKSE::GetTaskInterface()->AddTask(
                            [actorID, skillId, committedXP, gainedLevels, skillData]() {
                                auto target = RE::TESForm::LookupByID<RE::Actor>(actorID);
                                if (!target) return;
                                auto manager = Manager::GetSingleton();
                                manager->SetCustomSkillXP(target, skillId, committedXP);
                                manager->AddFollowerSkillLevelXP(target, skillData, gainedLevels);
                                Prisma::SendUpdateToUI();
                            });
                    })) {
                logger::warn(
                    "[SkillXP] Distribution unavailable actor={:08X} skill={}",
                    actor->GetFormID(),
                    skillId);
            }
            return;
        ...
        SetActorXP(actor, skillId, currentXP);
        AddFollowerSkillLevelXP(actor, data, gainedLevels);
...
void Manager::Save(SKSE::SerializationInterface* a_intfc) {
    ...
    if (!a_intfc->OpenRecord('APRG', 3)) return;
    ...
    for (const auto& [actorId, state] : actorProgressStates) {
        ...
        a_intfc->WriteRecordData(&state.skillLevelXP, sizeof(state.skillLevelXP));
    ...
    while (a_intfc->GetNextRecordInfo(type, version, length)) {
        ...
        if (type == 'APRG') {
            ...
            for (std::uint32_t i = 0; i < actorCount; ++i) {
                ...
                if (version >= 3 &&
                    !a_intfc->ReadRecordData(&state.skillLevelXP, sizeof(state.skillLevelXP))) break;
                ...
                if (actorResolved || !state.actorKey.empty()) {
                    ...
                    if (!std::isfinite(state.skillLevelXP) || state.skillLevelXP < 0.0f) state.skillLevelXP = 0.0f;
                    if (state.actorKey.empty()) {
